...
const Navigation: React.FC<NavigationProps> = ({ role, isDarkMode }) => {
  ...
  const NavItem = ({ path, icon: Icon, label }: { path: string, icon: any, label: string }) => {
    ...
    return (
      ...
      >
        <div className={`transition-all duration-500 transform ${isActive ? 'scale-110 translate-y-[-2px]' : 'opacity-80'}`}>
          <Icon className={`w-5 h-5 object-contain ${isActive ? 'text-cobalt drop-shadow-[0_0_8px_rgba(0,182,253,0.5)]' : 'text-app-muted'}`} />
        </div>
        <span className={`text-[10px] font-semibold mt-1 transition-all duration-300 ${isActive ? 'opacity-100 text-cobalt' : 'opacity-80'}`}>
          {label}
        </span>
        {isActive && (
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[150] backdrop-blur-xl safe-pb border-t border-app bg-[var(--pg-glass-bg-main)] transition-all duration-500 shadow-2xl dark:shadow-blue-950/40 shadow-slate-200/50">
      <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-blue-400/20 to-transparent"></div>
      <div className="h-14 max-w-[480px] md:max-w-2xl lg:max-w-4xl mx-auto px-4">
          ...
          {isManagementRole ? (
            <>
              ...
              <button
                ...
                className="relative -top-3 flex flex-col items-center justify-center p-0 bg-transparent hover:scale-110 active:scale-95 transition-all outline-none"
              >
                <div className="relative group">
                  <div className="absolute inset-0 bg-cobalt/20 blur-xl rounded-full group-hover:bg-cobalt/30 transition-all"></div>
                  <Icons.LogoSymbol className="w-[52px] h-[52px] object-contain relative z-10 drop-shadow-[0_0_15px_rgba(0,182,253,0.8)]" />
                </div>
              </button>
            ...
            <>
              <NavItem path="/home" icon={Icons.Home} label="Início" />
              <NavItem path="/training" icon={Icons.Dumbbell} label="Treino" />
              ...
              <NavItem path="/experience" icon={Icons.Star} label="Experience" />
