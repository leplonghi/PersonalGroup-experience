...
const Navigation: React.FC<NavigationProps> = ({ role }) => {
    ...
    return (
      ...
      >
        <div className={`transition-all duration-500 transform ${isActive ? 'scale-110 translate-y-[-4px]' : 'opacity-80'}`}>
          <Icon className={`w-6 h-6 object-contain ${isActive ? 'text-pg-cobalt drop-shadow-[0_0_8px_rgba(37,99,235,0.5)]' : 'text-blue-300'}`} />
        </div>
        <span className={`text-[10px] font-bold uppercase tracking-widest mt-1 transition-all duration-300 ${isActive ? 'opacity-100 text-pg-cobalt font-black' : 'opacity-80'}`}>
          {label}
        </span>
        {isActive && (
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[150] backdrop-blur-xl pb-safe border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,50,0.8)]" style={{ backgroundColor: '#080838' }}>
      ...
      <div className="h-20 max-w-[480px] md:max-w-2xl lg:max-w-4xl mx-auto px-4">
        <nav className="h-full flex items-center justify-around">
          {isManagementRole ? (
            <>
              <NavItem path="/management" icon={Icons.LogoSymbol} label="Painel" />
            ...
            <>
              ...
              <NavItem path="/evolution" icon={Icons.TrendingUp} label="Evolução" />

              {/* Botão Central de Treino (Destaque Premium) */}
              <button
                onClick={() => navigate('/session')}
                className="relative -top-6 flex flex-col items-center justify-center h-16 w-16 rounded-full bg-gradient-to-tr from-blue-700 to-blue-400 border-4 border-[#080838] shadow-[0_8px_25px_rgba(37,99,235,0.6)] hover:scale-105 active:scale-95 transition-all outline-none"
              >
                <div className="absolute inset-0 bg-white/20 rounded-full blur-[2px]"></div>
                <Icons.Play className="w-6 h-6 text-white relative z-10 ml-1 fill-current" />
              </button>
